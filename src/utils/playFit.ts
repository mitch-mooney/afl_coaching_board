 ...
 *
 * The board answers this for the Play the coach has open, through
 * useOutOfBounds. This is the same question asked of a Play sitting on disk, so
 * the coach can set the Active Venue to Saturday's ground and triage a whole
 * playbook from the list instead of opening plays one at a time.
 *
 ...
 * `outOfBounds` — and both the on-board count and the list marker call it, so
 * the list can never disagree with the board it opens.
 */
